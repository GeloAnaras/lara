...
class ProductController extends Controller
{
    ...
    {
        $this->middleware(['ApiAuth', 'ApiIsAdmin'])->except(['index', 'show', 'indexCounts', 'indexFilters']);
    }
    ...
    public function indexCounts()
    {
        return response()->json("opop");
    }
    ...
    public function indexFilters($category)
    {
        ...
        return response()->json(compact(['prod','rangePrice']));
    }
...
//            сдесь добавление значений
    ...
    {
        try{
            $prod = Product::with(['images','description','attributes','values'])
                ->where('id','=',$id)->first();
        }catch (\Exception $e){
            return response()->json(['error' => $e->getMessage()]);
        }
